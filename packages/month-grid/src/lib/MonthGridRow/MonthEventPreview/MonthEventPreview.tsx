...
export const MonthEventPreview: FComponent<MonthEventPreview> = (props) => {
  function ItemAdaptor() {
    const yy = new MonthDraggingItem(
      props.item.event.id,
      props.item.dragedStartDate,
      props.item.dragedEndDate,
      props.item.eventSourceEnd,
      props.item.eventSourceStart,
      props.item.event
    )
    return yy
  }
  function isEventInRange() {
    return !props.dontCheackRange
      ? isDateIncludedInaRange(ItemAdaptor() as unknown as EventClass, props.startDate, props.endDate)
      : true
  }
