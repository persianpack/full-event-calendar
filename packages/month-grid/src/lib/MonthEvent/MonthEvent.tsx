...
export const MonthEvent:FComponent<EventPropss> = (props: EventPropss)=>{  
    ...
    function eventStyles(){
        return `${eventIsDragging()?';opacity:.7;':''};left:calc(${leftP()}00% + 5px);width:calc(${widthh}00% ${rightArrowClass(props.item, props.dateEnd) ? '- 6px' : '- 16px'  });${leftArrowClass(props.item  as unknown as EventClass, props.date)};${isRighted}`
    }
  ...
  export function getExtraRows(na: splittted,weekStartDate:Date,weekendDate:Date){
    let rowMatrix = [0,0,0,0,0,0,0]
    const arr = Object.keys(na).filter((item,i)=>{
      return i + 1 > 2
    })

    for (let index = 0; index < arr.length; index++) {
        const element = na[arr[index]];
        // console.log(element)
