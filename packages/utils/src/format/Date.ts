export function formatDDMMYYYY(date: Date, calendar: string = 'gregory') {
  ...
  export function formatDM(date: Date, calendar: string,locale:string) {
    return new Intl.DateTimeFormat(locale, { month: 'short', weekday: 'short', calendar }).format(date)
  }
  
  export function formatDD(date: Date, calendar: string,locale:string) {
    return new Intl.DateTimeFormat(locale, { day: '2-digit', calendar }).format(date)
  }
